...
object WidgetCalendarBitmapRenderer {
    ...
    // Do NOT cap too low: users can resize widgets much taller/wider than the original preview.
    // If we clamp aggressively, the bitmap stays small while the RemoteViews grows, and the
    // action buttons (anchored to the widget bottom) appear to drift "outside" the calendar card.
    // Use a generous cap to avoid excessive bitmap memory usage while still covering common
    // launcher resize ranges.
    private const val MAX_W_DP = 800
    private const val MAX_H_DP = 800
    ...
    fun render(
        ...
        if (locked) {
            // Simple locked card for FREE/PRO tiers (Calendar is LIFETIME)
            val radius = w * 0.10f
            ...
            val pad = w * 0.07f
            val y = pad + title.textSize
            canvas.drawText("Premium Calendar", pad, y, title)
            ...
            canvas.drawText("Upgrade to LIFETIME", pad, y + sub.textSize * 1.6f, sub)
            canvas.drawText("to unlock this widget", pad, y + sub.textSize * 3.0f, sub)

            val badgeH = h * 0.13f
            val badgeW = w * 0.36f
            val badgeRect = RectF(w - pad - badgeW, pad, w - pad, pad + badgeH)
            val badgePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = theme.accent2 }
            ...
            canvas.drawText("LIFETIME", badgeRect.centerX(), by, badgeText)
            return bitmap
        }
