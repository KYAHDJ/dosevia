...
class AppViewModel(private val context: Context) : ViewModel() {
    ...
    // Premium tier (dev-controlled for now; later written by Billing)
    private val _userTier = MutableStateFlow(UserTier.FREE)
    val userTier: StateFlow<UserTier> = _userTier
    ...
    private fun loadUserTier() {
        _userTier.value = PremiumAccess.readTier(appContext)
        WidgetEntitlementManager.apply(appContext, _userTier.value)
    }

    /**
     * DEV / PRE-BILLING: manually set a tier so you can test UI locking.
     *
     * Later: you will call this from your Billing integration once purchases
     * are verified/restored.
     */
    fun setUserTier(tier: UserTier) {
        PremiumAccess.writeTier(appContext, tier)
        _userTier.value = tier
        WidgetEntitlementManager.apply(appContext, tier)
        // widgets/themes/ads should react immediately
        PillWidget.requestUpdate(appContext)
        PillWidgetMedium.requestUpdate(appContext)
        PillWidgetCalendar.requestUpdate(appContext)
    }

    fun isProUnlocked(): Boolean = _userTier.value.hasPro()
    fun isLifetimeUnlocked(): Boolean = _userTier.value.hasLifetime()
    ...
    private fun applyAlarm(settings: ReminderSettings, days: List<DayData>) {
        // Always write today's reminder decision so background receivers stay accurate.
        // (Receivers can't safely parse the whole app state.)
        val schedPrefs = appContext.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)

        // One-time onboarding gate:
        // On fresh install (or after user clears app data), we must NOT schedule alarms
        // until the user has accepted Terms and continued (Skip or Sign in).
        if (!OnboardingPrefs.isCompleted(appContext)) {
            cancelAlarm(appContext)
            cancelPreReminder(appContext)
            cancelOverdueTick(appContext)
            schedPrefs.edit()
                .putBoolean(KEY_SHOULD_REMIND_TODAY, false)
                .putLong(KEY_REMIND_DATE_MS, normDate(Date()))
                .apply()
            return
        }
        ...
        if (isPlacebo && !settings.placeboReminder) {
            cancelAlarm(appContext)
            cancelPreReminder(appContext)
            cancelOverdueTick(appContext)
            schedPrefs.edit()
                .putBoolean(KEY_SHOULD_REMIND_TODAY, false)
                .putLong(KEY_REMIND_DATE_MS, today)
                .apply()
            return
        }
        if (todayPill?.status == PillStatus.TAKEN)  {
            cancelAlarm(appContext)
            cancelPreReminder(appContext)
            cancelOverdueTick(appContext)
            schedPrefs.edit()
        ...
        // Pre-reminder heads-up notification.
        schedulePreReminderFromPrefs(appContext)

        // If we're already past the reminder time and the user is still NOT_TAKEN,
        // start the strict overdue tick loop (every minute) immediately.
        val dueAt = Calendar.getInstance().apply {
            set(Calendar.HOUR_OF_DAY, settings.dailyReminderHour)
            set(Calendar.MINUTE, settings.dailyReminderMinute)
            set(Calendar.SECOND, 0)
            set(Calendar.MILLISECOND, 0)
        }.timeInMillis
        if (System.currentTimeMillis() >= dueAt) {
            // Fire quickly; the receiver will then keep scheduling every minute.
            scheduleOverdueTick(appContext, System.currentTimeMillis() + 5_000L)
        } else {
            // Ensure first tick is at the due time.
    ...
    private fun checkAlarmTakenFlag() {
        ...
        val takenDate = prefs.getLong("alarm_taken_date", -1L)
        ...
        prefs.edit().remove("alarm_taken_date").commit()
    }
    ...
    private fun loadState() {
        ...
        // Notes can be large and are user-generated. If parsing fails for any reason
        // (e.g., corrupted JSON), do NOT wipe the in-memory notes; fall back to the
        // current state's notes so the UI doesn't appear to "lose" notes.
        val notesJson = configPrefs.getString(KEY_NOTES, null)
        val notes: List<Note> = notesJson?.let {
            try {
                val type = object : com.google.gson.reflect.TypeToken<List<Note>>() {}.type
                gson.fromJson<List<Note>>(it, type)
            } catch (_: Exception) {
                _state.value.notes
            }
        } ?: emptyList()
    ...
    /**
     * Notes are frequently updated and are the #1 place users notice any "didn't save" issues.
     * Persist them explicitly, independent of other config writes.
     */
    private fun saveNotesOnly() {
        configPrefs.edit().putString(KEY_NOTES, gson.toJson(_state.value.notes)).commit()
        cloudSyncManager.requestSyncDebounced()
    }
    ...
    fun updateDayStatus(day: Int, status: PillStatus) {
        ...
        if (status == PillStatus.TAKEN) {
            AchievementsManager.recordPillTaken(appContext)
        }
    ...
    fun addNote(content: String, date: Date, time: String) {
        ...
        AchievementsManager.recordNoteAdded(appContext)
        // Save notes explicitly first so they can't be clobbered by any other config write.
        saveNotesOnly()
        saveConfig()
    }
    ...
    fun clearBlister() {
        ...
        // Reset start date to today (normalized) to match a fresh blister start
        val today = normDate(Date()).let { Date(it) }
        configPrefs.edit().putLong("startDate", today.time).commit()

        Log.d("ClearBlister", "Cleared and reset startDate")
        reloadFromPrefs()
        cloudSyncManager.requestSyncDebounced()
    }
