...
class OverdueTickReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // Keep fast repeats while the device is active, but slow down while deeply idle.
...
fun schedulePreReminderFromPrefs(context: Context, leadMinutes: Int = DEFAULT_PRE_REMINDER_MINUTES) {
    ...
    AlarmReliability.scheduleBackgroundAlarm(
        context = context,
        triggerAtMillis = trigger,
        operation = pi,
        showIntent = buildShowAlarmPendingIntent(context),
...
fun nextOverdueDelayMs(context: Context): Long {
    ...
    return if (interactive && !idle) 60_000L else 9L * 60_000L
}
